...
- latitude (float64)
- longitude (float64)
- elevation_m (float64, placeholder default because TIFFs do not store elevation)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert SPHY GeoTIFF layers to parquet.")
    parser.add_argument(
        "--source-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=500.0,
        help=(
            "Fallback elevation_m assigned to every pixel because the current "
            "GeoTIFFs do not include elevation (default: 500.0)"
        ),
    )
    parser.add_argument(
        "--valid-mode",
...
def parse_filename(tif_path: Path) -> Tuple[str, str]:
    ...
    if len(parts) != 2:
        raise ValueError(
            f"Unexpected filename '{tif_path.name}'. Expected format like GMel_20110715.tif"
        )
    variable, date_token = parts
    if len(date_token) != 8 or not date_token.isdigit():
        raise ValueError(
            f"Unexpected date token '{date_token}' in '{tif_path.name}'. Expected YYYYMMDD."
        )
...
def validate_group(date_token: str, variable_files: Dict[str, Path]) -> None:
    ...
    for variable, tif_path in refs[1:]:
        ...
        if not same_grid:
            raise ValueError(
                f"Grid mismatch for date {date_token}: '{tif_path.name}' does not match "
                f"'{refs[0][1].name}'"
            )


def build_common_mask(arrays: Dict[str, np.ma.MaskedArray], valid_mode: str) -> np.ndarray:
...
def convert_group_to_frame(
    ...
    if transform is None:
        raise ValueError(f"Could not read raster transform for date {date_token}")
    ...
    longitudes, latitudes = xy(transform, valid_rows, valid_cols, offset="center")
    frame = pd.DataFrame(
        {
            ...
            "elevation_m": np.full(valid_rows.shape[0], float(default_elevation), dtype=np.float64),
        }
    )

    for variable, array in sorted(arrays.items()):
        values = array.data[valid_rows, valid_cols].astype(np.float64, copy=False)
        if valid_mode == "union":
...
def write_frame_to_parquet(frame: pd.DataFrame, output_file: Path) -> None:
    table = pa.Table.from_pandas(frame, preserve_index=False)
    metadata = dict(table.schema.metadata or {})
    for key, value in frame.attrs.items():
        metadata[f"sphy_{key}".encode("utf-8")] = str(value).encode("utf-8")
...
def convert_all(
    ...
    logger.info("Default elevation_m: %.2f", default_elevation)
    logger.info("=" * 72)

    for idx, (date_token, variable_files) in enumerate(sorted(grouped.items()), start=1):
        output_file = output_dir / f"SPHY_{date_token}.parquet"
        if output_file.exists() and not overwrite:
            logger.info("[%d/%d] Skipping %s (already exists)", idx, len(grouped), output_file.name)
